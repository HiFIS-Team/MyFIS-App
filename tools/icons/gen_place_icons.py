...
# ── 편의시설 ────────────────────────────────────────────────────────────────
# 화장실 — 만국 공통 표지. 왼쪽은 바지, 오른쪽은 치마다
TOILET = " ".join([
    "M7,4.6 v0.01",             # 왼쪽 머리 (둥근 끝 = 점)
    "M7,8 v4.5",                # 몸
    "M4.9,9.4 h4.2",            # 어깨
    "M5.6,12.5 v6.4",           # 다리
    "M8.4,12.5 v6.4",
    "M17,4.6 v0.01",            # 오른쪽 머리
    "M17,8 L14.6,13.4 h4.8 z",  # 치마 — 삼각형이라야 갈라 읽힌다
    "M15.7,13.4 v5.5",          # 다리
    "M18.3,13.4 v5.5",
])
...
# 탈의실 — 사물함. **가로 칸 + 손잡이 둘**이 문 두 짝을 만든다.
# 통풍구는 한 줄만 둔다. 두 줄을 겹치면 28px 에서 얼룩이 된다
LOCKER = " ".join([
    "M5,4.4 h14 v15.2 h-14 z",  # 몸통
    "M5,12 h14",                # 위·아래 문을 가르는 선
    "M8,7.8 h4",                # 통풍구 — 위 문에만
    "M16.2,8.4 v2.2",           # 위 문 손잡이
    "M16.2,13.4 v2.2",          # 아래 문 손잡이
])
